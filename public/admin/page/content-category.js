...
$(document).ready(function () {
    ...
    // Load datatable
    $('#content-cat-show').DataTable({
        processing: true,
        serverSide: true,
        ajax: routeUrl + '/admin/content-category',
        columnDefs: [
            {
                targets: 0,
                checkboxes: {
...
$(document).on('click', '.add_form', function (e) {
    ...
    $('.modal-title').html('Add content category');
    $('#data_form')[0].reset();
    $('#id').val('');
...
$(document).on('click', '.edit_form', function (e) {
    ...
    $.ajax({
        ...
        success: (data) => {
            if (data.status == true) {
                $('#modal_div').modal({ backdrop: 'static', keyboard: false }, 'show');
                $('.modal-title').html('Edit content category');
                $('#data_form')[0].reset();
                $('#id').val(data.data.id);
...
function submit_form() {
    $("#data_form").validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                type: 'POST',
                url: routeUrl + '/admin/content-category',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: (data) => {
                    if (data.status == false) {
                        ...
                        var oTable = $('#content-cat-show').dataTable();
                        oTable.fnDraw(false);
                    }
                },
                error: function (data) {
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res != '') {
                ...
                $('.modal-title').html('Content category details');
            } else {
...
// Delete single content category
        ...
        if (isConfirm.isConfirmed == true) {
            $.ajax({
                ...
                success: (data) => {
                    if (data.status == true) {
                        toastr.success(data.message, 'Success');
                        var oTable = eval($('#content-cat-show').dataTable());
                        oTable.fnDraw(false);
                    } else if (data.status == false) {
                        toastr.error(data.message);
                    }
...
// Delete multiple content category
$(document).on('click', '#multiple_delete_btn', function (e) {
    var table = $('#content-cat-show').dataTable();
    ...
    if (rows_selected.length > 0) {
        Swal.fire({
            ...
            if (isConfirm.isConfirmed == true) {
                ...
                $.ajax({
                    type: 'POST',
                    url: routeUrl + '/admin/delete-content-category',
...
// content category status change
$(document).on('click', '.change-status-record', function (e) {
    ...
    Swal.fire({
        ...
        if (isConfirm.isConfirmed == true) {
            ...
            $.ajax({
                type: 'post',
                url: routeUrl + '/admin/content-category/status-update',
                data: { 'id': id },
                success: (data) => {
                    if (data.status == true) {
                        toastr.success(data.message, 'Success');
